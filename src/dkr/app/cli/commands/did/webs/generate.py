...
parser = argparse.ArgumentParser(
    description="Generate a did:webs DID document and KEL/TEL file"
)
...
parser.add_argument(
    "--did", "-d", help="DID to generate (did:webs method)", required=True
)
...
class Generator(doing.DoDoer):
    ...
    def generate(self, tymth, tock=0.0, **opts):
        ...
        # generate did doc
        result = didding.generateDIDDoc(
            self.hby,
            did=self.did,
            aid=aid,
            oobi=None,
            reg_name=self.da_reg,
            meta=self.meta,
        )
        ...
        result = dict(didDocument=diddoc, pre=pre, state=state, kel=kel)
