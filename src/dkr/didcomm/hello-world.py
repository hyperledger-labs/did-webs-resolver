...
store = {alice["did"]: alice, bob["did"]: bob}
...
alice_message_packed = asyncio.run(
    pack_encrypted(
        resolvers_config=ResolversConfig(
            secrets_resolver=secrets_resolver, did_resolver=did_resolver
        ),
...
bob_message_unpacked = asyncio.run(
    unpack(
        resolvers_config=ResolversConfig(
            secrets_resolver=secrets_resolver, did_resolver=did_resolver
        ),
...
bob_message_packed = asyncio.run(
    pack_encrypted(
        resolvers_config=ResolversConfig(
            secrets_resolver=secrets_resolver, did_resolver=did_resolver
        ),
...
alice_message_unpacked = asyncio.run(
    unpack(
        resolvers_config=ResolversConfig(
            secrets_resolver=secrets_resolver, did_resolver=did_resolver
        ),
...
print(
    "6-Alice decrypts the message with short DIDs:",
    alice_message_unpacked.message.body,
    "\n",
)
