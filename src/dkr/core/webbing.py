...
def setup(app, hby, cf):
    ...
    if DD_DIR_CFG in data:
        print(
            f"Using config property {DD_DIR_CFG} to look for {DID_JSON} files: {data[DD_DIR_CFG]}"
        )
    ...
    if KC_DIR_CFG in data:
        print(
            f"Using config property {KC_DIR_CFG} to look for {KERI_CESR} files: {data[KC_DIR_CFG]}"
        )
    ...
    def on_get(self, req, rep, aid):
        ...
        if not req.path.endswith(f"/{DID_JSON}"):
            raise falcon.HTTPBadRequest(
                description=f"invalid {DID_JSON} DID URL {req.path}"
            )
        ...
        if not aid in self.lookup:
            raise falcon.HTTPNotFound(
                description=f"{DID_JSON} for KERI AID {aid} not found"
            )
...
class KeriCesrWebResourceEnd(DIDWebResourceEnd):
    ...
    def add_lookup(self, aid, fpath):
        ...
        with open(fpath, "rb") as file:
            self.hby.psr.parse(ims=bytearray(file.read()), local=True)
            if aid and aid in self.hby.kevers:
                print(
                    f"KERI CESR parsing {fpath} succeeded, KERI AID {aid} found in habery"
                )
                self.lookup[aid] = fpath
            else:
                print(
                    f"FAILED: KERI CESR parsing {fpath} failed, KERI AID {aid} not found in habery"
                )
    ...
    def on_get(self, req, rep, aid):
        ...
        if not req.path.endswith(f"/{KERI_CESR}"):
            raise falcon.HTTPBadRequest(
                description=f"invalid {KERI_CESR} DID URL {req.path}"
            )
        ...
        if not aid in self.lookup:
            raise falcon.HTTPNotFound(
                description=f"keri.cesr for KERI AID {aid} not found"
            )
