...
DID_WEBS_RE = re.compile(
    r"\Adid:web(s)?:(?P<domain>[^%:]+)(?:%3a(?P<port>\d+))?(?::(?P<path>.+?))?(?::(?P<aid>[^:]+))\Z",
    re.IGNORECASE,
)
...
def generateDIDDoc(hby: habbing.Habery, did, aid, oobi=None, meta=False, reg_name=None):
    ...
    if aid in hby.kevers:
        ...
        print(
            f"Habery does not have a kever for {did}. Did you parse the keri.cesr file?"
        )
    ...
    if isinstance(kever.tholder.thold, int):
        ...
        numerators = [
            int(fr.numerator * lcd / fr.denominator) for fr in kever.tholder.thold[0]
        ]
        conditions = []
        for idx, verfer in enumerate(kever.verfers):
    ...
    x = [
        (keys[1], loc.url)
        for keys, loc in hby.db.locs.getItemIter(keys=(aid,))
        if loc.url
    ]
...
def toDidWeb(diddoc):
    ...
    for verificationMethod in diddoc[VMETH_FIELD]:
        verificationMethod["controller"] = verificationMethod["controller"].replace(
            "did:webs", "did:web"
        )
    return diddoc
...
def desAliases(hby: habbing.Habery, aid: str, reg_name: str = None):
    ...
    if aid in hby.habs:
        ...
        saids = [
            saider
            for saider in saids
            if saider.qb64 in [saider.qb64 for saider in scads]
        ]
        ...
        for idx, cred in enumerate(creds):
            ...
            if status["et"] == "iss" or status["et"] == "bis":
                ...
                print(
                    f"    Status: Revoked {terming.Colors.FAIL}{terming.Symbols.FAILED}{terming.Colors.ENDC}"
                )
            else:
        ...
        for eList in rList:
            for eid in eList:
                ...
                for proto in val:
                    host = val[proto]
                    sDict[proto] = f"{host}"
                    v = dict(id=f"#{eid}/{role}", type=role, serviceEndpoint=sDict)
                    if v not in sEnds:
                        sEnds.append(v)
    return sEnds
